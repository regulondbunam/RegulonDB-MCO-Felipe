...
def set_identifier_object(json_object, collection_name,  **metadata_properties):
    metadata_properties["type"] = collection_name #if collection_name not in ["ontologies", "terms"] else "ontologies"
    metadata_properties["uniqueDataString"] = get_unique_data[collection_name](**json_object)

    object_id = json_object["_id"]

    identifier_object = build_identifier_object(object_id, **metadata_properties)
