...
def replace_mg_identifiers(jsons_data, regulondb_version, organism, output_path=None):
    multigenomicdb_identifiers = identifiers_api.regulondbmultigenomic.get_all_identifiers(regulondb_version, organism)
    for filename, dataset in jsons_data.items():
        ...
        for json_object in collection_data:
            mg_replace_ids_builder[collection_name](json_object, multigenomicdb_identifiers, collection_name)
...
def set_log(log_path):
    ...
    logging.basicConfig(filename=os.path.join(log_path, 'replace_identifiers.log'), format='%(levelname)s - %(asctime)s - %(message)s', filemode='w', level=logging.INFO)
...
def run(regulondb_version, organism, input_path, output_path, database):
    ...
    if database == 'regulondbmultigenomic':
        replace_mg_identifiers(input_files, regulondb_version, organism, output_path)
...
if __name__ == '__main__':
    ...
    run(arguments.version, arguments.organism, arguments.inputdir, arguments.outputdir, arguments.database)
